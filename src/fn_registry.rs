...
pub type DecompressFn = Arc<dyn Fn(Bytes, &[u8]) -> Vec<(Timestamp, Bytes)> + Send + Sync>;
...
pub type CompressDispatchFn = Arc<dyn Fn(&[u8]) -> &str + Send + Sync>;
...
impl FnRegistry {
    ...
    pub fn compress_entries(&self, key: Bytes, data: Vec<(i64, Vec<u8>)>) -> Result<Bytes> {
        let compress_fn_name = self.dispatch_fn()(&key);
        let compress_fn = self.udcf(compress_fn_name)?.compress();
        Ok(compress_fn(key.clone(), data))
    }

    pub fn decompress_entries(&self, key: &[u8], data: &[u8]) -> Result<Vec<(Timestamp, Bytes)>> {
...
///
/// Compress: first put all entries' bytes together. Then followed a block of
/// bytes records each entry's length in u64. The last 8 bytes is how many
/// entries sited. ```text
...
pub fn noop_udcf() -> UDCF {
    ...
    let decompress_fn: DecompressFn = Arc::new(|_key, raw_values| {
        let mut raw_values: VecDeque<u8> = raw_values.iter().cloned().collect();
