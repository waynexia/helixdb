...
impl Levels {
    pub async fn try_new(
        ...
        let cache = Cache::with_config(opts.cache);
        let write_batch = WriteBatch::with_config(opts.write_batch);
...
pub struct WriteBatchConfig {
    /// The maximum number of entries can be hold in one batch.
    pub batch_size: usize,
    /// The longest time duration between two batch consumptions.
...
struct WriteBatch {
    ...
    timeout: Duration,
    batch_size: usize,
    /// Lock on two vectors `notifier` and `buf`.
    lock: Mutex<()>,
...
impl WriteBatch {
    pub fn with_config(config: WriteBatchConfig) -> Self {
        Self {
    ...
    pub fn default() -> Self {
        Self::with_config(WriteBatchConfig::default())
    }
